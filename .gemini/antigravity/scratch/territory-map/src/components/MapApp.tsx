...
import { fetchSheetData, MarkerData } from "@/lib/googleSheets";
...
export default function MapApp() {
  ...
  const loadData = async () => {
    ...
    try {
      const [sheetData, geojsonData] = await Promise.all([
        fetchSheetData(),
        fetch("/counties.geojson").then(res => res.json()).catch(() => null)
      ]);
      setMarkers(sheetData);
      if (geojsonData) {
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-[#f8fafc] overflow-hidden text-gray-900">
      ...
      <main className="flex-1 flex overflow-hidden relative">
        {/* Sidebar */}
        <Sidebar markers={markers} onRefresh={loadData} isLoading={isLoadingData} />
        
        {/* Map */}
        <div className="flex-1 relative z-0">
          <MapComponent markers={markers} geojson={geojson} />
        </div>
      </main>
    </div>
  );
